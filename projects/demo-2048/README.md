...
2) Run intake and answer questions:
   bash scripts/tick.sh --project projects/my-project
...
- Watch current progress:
  bash scripts/status.sh --tail 50 --project projects/my-project
- Refresh every second:
  watch -n 1 bash scripts/status.sh --tail 50 --project projects/my-project
...
- New projects include only per-project data files (SPEC/TASKS/STATE/ROLES/INTAKE/notes/logs).
- Core scripts and UI stay in the repo root and are shared by all projects.
- New projects start at INTAKE_READY to gather requirements and roles.

Project isolation
- Each project lives under projects/ in its own folder.
- STATE.json, TASKS.yaml, logs/, notes/, and .agent_lock are local to that folder.
...
Suggested watcher (macOS/Linux with entr):
  ls projects/my-project/SPEC.md projects/my-project/TASKS.yaml projects/my-project/STATE.json | entr -r bash scripts/tick.sh --project projects/my-project

Suggested timer loop:
  while true; do bash scripts/tick.sh --project projects/my-project; sleep 10; done
